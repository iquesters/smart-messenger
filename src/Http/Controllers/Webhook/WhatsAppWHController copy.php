...
class WhatsAppWHController extends Controller
{
    ...
    public function handle(Request $request, string $channelUid)
    {
        ...
        try {
            ...
            /**
             * Api request response logging should go to middleware 
             * Incoming message job dispatch (Incomming message processor job)
             * return 200
             * 
             * Base webhook cntroller sync/async=? 
             * if async=T, provide a job:basejob() dispatch
             * BaseJob->BaseWebhookJob->WhatsappWebHookJob(type of message IncommingMessage)
             * if async=false, Jobs::handle
             */
